...
begin
  ...
  with product_items as (
    ...
    where item.item_type_snapshot = 'product'
  ...
  left join public.inventory_stock as stock
    on stock.catalog_item_id = pi.item_id
   and stock.tenant_id = v_tenant_id
   and stock.branch_id = v_resolved_branch_id
  where stock.id is null
     or stock.quantity_on_hand < pi.requested_quantity
  order by pi.item_name_snapshot asc
  limit 1;
  ...
  join (
    ...
    on product_items.item_id = stock.catalog_item_id
  ...
  set quantity_on_hand = stock.quantity_on_hand - product_items.requested_quantity
  from product_items
  where stock.tenant_id = v_tenant_id
    and stock.branch_id = v_resolved_branch_id
    and stock.catalog_item_id = product_items.item_id;
  ...
  select
    ...
    'sale',
    -product_items.requested_quantity,
    'invoice',
    v_invoice_id,
