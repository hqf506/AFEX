...
select
  ...
  coalesce(
    ...
    case when im.movement_type = 'sale' then 'النظام' end,
    '-'
  ) as user_name,
  coalesce(
    nullif(trim(p.full_name), ''),
  ...
  case
    ...
    when p.id is not null and coalesce(to_jsonb(p)->>'role', '') = 'owner' then 'owner'
    when p.id is not null then 'admin'
    when im.movement_type = 'sale' then 'system'
    else 'unknown'
  end as actor_type
