...
alter table public.branch_catalog_items
add column if not exists tenant_id uuid;

alter table public.branch_whatsapp_configs
add column if not exists tenant_id uuid;
...
create or replace function public.resolve_branch_tenant_id(p_branch_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  ...
  return coalesce(v_tenant_id, public.resolve_default_tenant_id());
end;
$$;

create or replace function public.set_branches_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.set_profiles_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.tenant_id is null then
    new.tenant_id := public.resolve_branch_tenant_id(new.branch_id);
  end if;
...
create or replace function public.set_customers_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.set_orders_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if new.tenant_id is null and new.created_by_employee_id is not null then
    select tenant_id
    into new.tenant_id
    from public.profiles
    where id = new.created_by_employee_id
    limit 1;
  end if;
...
create or replace function public.set_invoices_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.set_invoice_items_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
create or replace function public.set_branch_scoped_tenant_id()
returns trigger
language plpgsql
security definer
set search_path = public
...
drop trigger if exists trg_set_branches_tenant_id on public.branches;
create trigger trg_set_branches_tenant_id
before insert on public.branches
for each row
execute function public.set_branches_tenant_id();

drop trigger if exists trg_set_profiles_tenant_id on public.profiles;
create trigger trg_set_profiles_tenant_id
before insert on public.profiles
for each row
execute function public.set_profiles_tenant_id();

drop trigger if exists trg_set_customers_tenant_id on public.customers;
create trigger trg_set_customers_tenant_id
before insert on public.customers
for each row
execute function public.set_customers_tenant_id();

drop trigger if exists trg_set_orders_tenant_id on public.orders;
create trigger trg_set_orders_tenant_id
before insert on public.orders
for each row
execute function public.set_orders_tenant_id();

drop trigger if exists trg_set_invoices_tenant_id on public.invoices;
create trigger trg_set_invoices_tenant_id
before insert on public.invoices
for each row
execute function public.set_invoices_tenant_id();

drop trigger if exists trg_set_invoice_items_tenant_id on public.invoice_items;
create trigger trg_set_invoice_items_tenant_id
before insert on public.invoice_items
for each row
execute function public.set_invoice_items_tenant_id();

drop trigger if exists trg_set_catalog_items_tenant_id on public.catalog_items;
create trigger trg_set_catalog_items_tenant_id
before insert on public.catalog_items
for each row
execute function public.set_default_tenant_id();

drop trigger if exists trg_set_catalog_categories_tenant_id on public.catalog_categories;
create trigger trg_set_catalog_categories_tenant_id
before insert on public.catalog_categories
for each row
execute function public.set_default_tenant_id();

drop trigger if exists trg_set_branch_catalog_items_tenant_id on public.branch_catalog_items;
create trigger trg_set_branch_catalog_items_tenant_id
before insert on public.branch_catalog_items
for each row
execute function public.set_branch_scoped_tenant_id();

drop trigger if exists trg_set_branch_whatsapp_configs_tenant_id on public.branch_whatsapp_configs;
create trigger trg_set_branch_whatsapp_configs_tenant_id
before insert on public.branch_whatsapp_configs
for each row
execute function public.set_branch_scoped_tenant_id();

drop trigger if exists trg_set_discounts_tenant_id on public.discounts;
create trigger trg_set_discounts_tenant_id
before insert on public.discounts
for each row
execute function public.set_branch_scoped_tenant_id();

drop trigger if exists trg_set_vat_settings_tenant_id on public.vat_settings;
create trigger trg_set_vat_settings_tenant_id
before insert on public.vat_settings
for each row
execute function public.set_branch_scoped_tenant_id();

drop trigger if exists trg_set_system_settings_tenant_id on public.system_settings;
create trigger trg_set_system_settings_tenant_id
