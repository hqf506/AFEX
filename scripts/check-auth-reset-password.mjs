...
assert(callback.includes("console.warn('[auth-recovery-callback]', category)") && callback.includes('PKCE_EXCHANGE_FAILED') && callback.includes('RECOVERY_CALLBACK_HOST_MISMATCH'), 'Callback diagnostics must use fixed safe categories only.')
assert(!callback.includes('error.message') && !callback.includes('request.nextUrl.toString()'), 'Callback must not log codes, URLs, or expose raw errors.')
...
assert(recovery.includes('CALLBACK_STATE_MAX_AGE_SECONDS = 15 * 60') && recovery.includes('RECOVERY_MAX_AGE_SECONDS = 10 * 60'), 'Signed state must last 15 minutes and post-callback context must remain shorter.')
...
assert(callback.includes('hasValidRecoveryCallbackStateSignature(state)') && complete.includes('clearRecoveryContext()'), 'Expired or reused recovery authorization must fail safely without a custom token store.')
...
assert(!request.includes('AUTH_RECOVERY_REQUEST_ACCEPTED'), 'Temporary recovery success diagnostics must not remain enabled.')
