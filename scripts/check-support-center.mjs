...
const errorReportRoute = read('app/api/support/error-reports/route.ts')
const errorReportRequest = read('lib/support/error-report-request.ts')
const customerSupportList = read('app/admin/support/page.tsx')
const customerSupportDetail = read('app/admin/support/[id]/page.tsx')
const providerTicketDetails = read('components/provider-ticket-details.tsx')
...
assert(!listRoute.match(/const TICKET_SELECT = .*tenant_id|const TICKET_SELECT = .*branch_id|const TICKET_SELECT = .*assigned_to/), 'Customer ticket lists must not serialize tenant, branch, or assignment identifiers.')
...
assert(errorFallback.includes("fetch('/api/support/error-reports'") && errorReportRoute.includes('createSupportTicket'), 'Crash reports must use the strict endpoint and trusted Support ticket path.')
assert(listRoute.includes('isTrustedErrorReportRequest(request)') && listRoute.includes("const source = isErrorReport ? 'error_report' : 'manual'"), 'Customer ticket creation must not allow browser-controlled system or error-report sources.')
assert(errorReportRoute.includes('markTrustedErrorReportRequest(supportRequest)') && errorReportRequest.includes('WeakSet<NextRequest>'), 'Only the strict error-report endpoint may enable automatic report creation.')
assert(messagesRoute.includes("ticket.status === 'closed'") && messagesRoute.includes('لا يمكن الرد على تذكرة مغلقة'), 'Closed-ticket reply restrictions must be enforced by the server API.')
...
assert(errorReportRoute.includes('ALLOWED_KEYS') && errorReportRoute.includes('comment.trim().length > 1000'), 'Error report payloads must be strictly allowlisted and bounded.')
assert(!errorReportRoute.includes('tenant_id') && !errorReportRoute.includes('user_id'), 'Error report identity and tenant must never come from browser input.')
assert(listRoute.includes('error_fingerprint: errorFingerprint') && listRoute.includes("(rateResult.count || 0) >= 5"), 'Error reports must include safe fingerprints and lightweight abuse protection.')
assert(errorFallback.includes('result.reused') && errorFallback.includes('data?.error') && errorFallback.includes('/admin/support/${encodeURIComponent(result.ticket.id)}'), 'Error report UX must distinguish duplicates, preserve safe API errors, and link to the customer ticket.')
assert(customerSupportList.includes("ticket.source === 'error_report'") && customerSupportDetail.includes("ticket.source === 'error_report'") && providerTicketDetails.includes("ticket.source === 'error_report'"), 'Automatic reports must be identified consistently in customer and provider views.')
