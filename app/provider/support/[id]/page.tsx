...
export default function ProviderTicketDetailsPage() {
  ...
  const loadTicket = useCallback(async (signal?: AbortSignal) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/provider/support/tickets/${encodeURIComponent(params.id)}`, { signal, cache: 'no-store' })
  ...
  async function updateTicket(field: 'status' | 'priority' | 'category', value: string) {
    ...
    setNotice(null)
    try {
      const response = await fetch(`/api/provider/support/tickets/${encodeURIComponent(params.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function submitInternalNote(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice(null)
    try {
      const response = await fetch(`/api/provider/support/tickets/${encodeURIComponent(params.id)}/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!ticket) return <main dir="rtl" className="mx-auto max-w-4xl space-y-4"><AdminAlert tone="error">{error || 'تعذر تحميل تفاصيل تذكرة الدعم.'}</AdminAlert><Link href="/provider/support" className="inline-flex h-11 items-center rounded-2xl border border-cyan-300/20 px-5 text-sm font-black text-cyan-100">العودة إلى لوحة الدعم</Link></main>

  const replyDisabled = ticket.status === 'closed'

  return (
    <main dir="rtl" className="mx-auto w-full min-w-0 max-w-[1450px] space-y-5">
      <Link href="/provider/support" className="inline-flex h-10 items-center rounded-xl border border-white/10 px-4 text-xs font-black text-slate-300 transition hover:border-cyan-300/30 hover:text-white">العودة إلى التذاكر</Link>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(300px,0.8fr)]">
        ...
        <aside className="min-w-0 space-y-5">
          ...
          <AdminGlassSection><h2 className="text-lg font-black text-white">بيانات التذكرة</h2><dl className="mt-4 divide-y divide-white/[0.07] text-sm">{[['المنشأة', ticket.tenant_name], ...(ticket.branch_name ? [['الفرع', ticket.branch_name]] : []), ['العميل', ticket.customer_name], ['التصنيف', supportCategoryLabels[ticket.category]], ['المصدر', supportSourceLabels[ticket.source]], ['تاريخ الإنشاء', formatSupportDate(ticket.created_at)], ['آخر تحديث', formatSupportDate(ticket.updated_at)]].map(([label, value]) => <div key={label} className="flex min-w-0 items-start justify-between gap-4 py-3"><dt className="shrink-0 text-slate-500">{label}</dt><dd className="min-w-0 break-words text-left font-bold text-slate-200">{value}</dd></div>)}</dl></AdminGlassSection>
