...
export default function PosOrderHistoryPage() {
  ...
  return <div className={styles.page} dir="rtl"><main className={styles.main}>
    ...
    {!error && filtered.length > 0 ? <section className={styles.timeline} aria-label="سجل عمليات اليوم"><section><h2 className={styles.day}>{getRiyadhDayLabel()}</h2>{filtered.map((operation) => <article className={styles.item} key={operation.id} aria-label={`فتح تفاصيل ${operation.reference}`} onClick={(event) => void openDetails(operation.order, event.currentTarget)}><time className={styles.desktopTime}>{formatPosOperationTime(operation.createdAt)}</time><div className={styles.marker}><InvoiceIcon /></div><div className={styles.card}><div className={styles.operation}><strong>{operation.title}</strong><p>{operation.description}</p></div><small className={styles.customer}>{operation.customerName || 'عميل نقدي'}</small><time className={styles.mobileTime}>{formatPosOperationTime(operation.createdAt)}</time><small className={styles.reference} dir="ltr">{operation.reference}</small><span className={`${styles.status} ${styles[operation.statusTone]}`}>{operation.statusLabel}</span><button className={styles.detailsAction} type="button" aria-label={`عرض تفاصيل ${operation.reference}`} onClick={(event) => { event.stopPropagation(); void openDetails(operation.order, event.currentTarget) }}><DetailsIcon /><span>عرض التفاصيل</span></button></div></article>)}</section></section> : null}
  ...
  {selected ? <div className={styles.dialogBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) closeDetails() }}><section ref={sheetRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="pos-invoice-sheet-title"><header className={styles.dialogHeader}><div><small>تفاصيل الطلب</small><h2 id="pos-invoice-sheet-title" dir="ltr">{selected.order_number} / {selected.invoice_number}</h2></div><button className={styles.dialogClose} type="button" onClick={closeDetails} aria-label="إغلاق تفاصيل الطلب"><span aria-hidden="true">×</span><span>إغلاق</span></button></header><div className={styles.dialogBody}>
