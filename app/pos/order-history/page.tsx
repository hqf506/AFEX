...
export default function PosOrderHistoryPage() {
  ...
  return <div className="pos-invoice-history pos-order-history-page" dir="rtl"><main>
    <div className="pos-order-history-controls pos-operations-controls">
      <header className="pos-history-header pos-operations-header"><div className="pos-history-heading"><span><InvoiceIcon /></span><div><h1>سجل العمليات</h1><p>نشاطك في نقطة البيع</p></div></div><span className="pos-operations-employee"><UserIcon />{employeeName}</span><div className="pos-operations-header-actions"><button type="button" onClick={() => void loadInvoices(1)} disabled={loading}><RefreshIcon /><span>تحديث</span></button><button type="button" onClick={() => router.push('/pos')} aria-label="إغلاق سجل العمليات"><span aria-hidden="true">×</span><span>إغلاق</span></button></div></header>
      <div className="pos-operations-stats" aria-label="إحصائيات العمليات"><div><UserIcon /><small>عملاء</small><strong>{customerCount}</strong></div><div><InvoiceIcon /><small>فواتير</small><strong>{todayCount}</strong></div><div><ActivityIcon /><small>عمليات اليوم</small><strong>{todayCount}</strong></div></div>
