...
function SalesByItemPlaceholder() {
  return (
...
export default function SalesByItemPage() {
  ...
  const sortedItemRows = useMemo(() => {
    ...
    return [...itemRows].sort((left, right) => {
      let comparison = 0

      if (sortKey === 'itemName' || sortKey === 'itemType') {
  ...
  }, [itemRows, sortDirection, sortKey])
  ...
  const visibleTopSellingRows = useMemo(
    () => (showAllItems ? topSellingRows : topSellingRows.slice(0, 5)),
    [showAllItems, topSellingRows]
  )

  const totalQuantitySold = useMemo(
    () => itemRows.reduce((sum, row) => sum + row.quantitySold, 0),
    [itemRows]
  )

  const totalGrossSales = useMemo(
    () => itemRows.reduce((sum, row) => sum + row.grossSales, 0),
    [itemRows]
  )

  const totalProfit = useMemo(
    () => itemRows.reduce((sum, row) => sum + row.profit, 0),
    [itemRows]
  )

  const rankByItemKey = useMemo(() => {
    const rankMap = new Map<string, number>()
    topSellingRows.forEach((row, index) => {
      rankMap.set(row.itemKey, index + 1)
    })
    return rankMap
  }, [topSellingRows])
  ...
  const metricCards = useMemo(
    () => [
      ...
      {
        title: 'عدد العناصر',
        value: itemRows.length.toString(),
        hint: 'عنصر',
        icon: 'items',
      },
      {
    ...
    [itemRows.length, period, totalGrossSales, totalProfit, totalQuantitySold]
  )

  function getItemShare(rowGrossSales: number) {
  ...
  function getItemPerformanceBadge(rank: number, share: number, rowGrossSales: number) {
  ...
  function handleSortChange(nextSortKey: SalesByItemSortKey) {
    setSortKey((currentSortKey) => {
      ...
      setSortDirection(
        nextSortKey === 'itemName' || nextSortKey === 'itemType' ? 'asc' : 'desc'
      )
      return nextSortKey
    })
  }

  function getSortIndicator(columnKey: SalesByItemSortKey) {
    if (sortKey !== columnKey) return '·'
    return sortDirection === 'asc' ? '↑' : '↓'
  }

  function exportSalesByItemCsv() {
    if (sortedItemRows.length === 0) return
    ...
    const rows = sortedItemRows.map((row) => [
      row.itemName,
      row.itemType,
      row.itemCategory,
      row.quantitySold,
    ...
    link.download = `sales-by-item-${filePeriod}-${dateFrom}-to-${dateTo}.csv`
    link.click()
  ...
  return (
    <div className="min-h-full overflow-hidden bg-[#020817] p-4 text-right text-white sm:p-6">
      ...
      <div className="w-full max-w-none space-y-4 rounded-[24px] border border-white/10 bg-white/[0.025] p-3 shadow-[0_28px_140px_rgba(0,0,0,0.34)] backdrop-blur sm:p-6 md:space-y-5 md:rounded-[32px]">
        <MobilePageHeader title="المبيعات حسب العنصر" subtitle={getPeriodLabel(period)} />
        {errorMessage ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-400">
                  نظرة مفصلة على أداء العناصر والمبيعات خلال الفترة الحالية.
                </p>
              </div>
            </div>
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.045] p-5 shadow-[0_24px_90px_rgba(0,0,0,0.20)] backdrop-blur">
          <div data-responsive-filters className="grid gap-4 xl:grid-cols-[minmax(0,260px)_minmax(0,1fr)_minmax(0,180px)_minmax(0,180px)_auto] xl:items-end">
            ...
            <div className="flex items-end justify-end gap-2 xl:justify-start">
              <button
                type="button"
                onClick={() => void fetchSalesByItemData()}
              ...
              <button
                ...
                disabled={sortedItemRows.length === 0}
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.045] p-5 shadow-[0_24px_90px_rgba(0,0,0,0.20)] backdrop-blur">
          <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-black text-white">العناصر الأكثر مبيعاً</h2>
              <p className="mt-1 text-sm text-slate-500">
                أفضل العناصر خلال {getPeriodLabel(period)} مرتبة حسب إجمالي البيع.
              </p>
            </div>
            <span className="rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-xs font-black text-slate-300">
              {topSellingRows.length} عنصر
            </span>
          </div>
            ...
            <>
              <div data-responsive-report-cards className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                {visibleTopSellingRows.map((row, index) => {
                  ...
                  const badge = getItemPerformanceBadge(rank, share, row.grossSales)
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-base font-black text-white">
                            {row.itemName || 'عنصر غير محدد'}
                          </p>
                          <p className="mt-1 truncate text-xs font-bold text-slate-500">
                            {row.itemCategory || row.itemType || 'بدون تصنيف'}
                          </p>
                        </div>
              ...
              {topSellingRows.length > 5 ? (
                <div className="mt-5 flex justify-center">
                  ...
                  >
                    {showAllItems ? 'عرض أقل ↑' : 'عرض كل العناصر ↓'}
                  </button>
                </div>
              ) : null}
            </>
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.045] p-5 shadow-[0_24px_90px_rgba(0,0,0,0.20)] backdrop-blur">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                مرجع تفصيلي لكل العناصر ضمن النتائج الحالية.
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[980px] text-[15px]">
                      ...
                      {[
                        ...
                        <th key={`${key}-${label}`} className="px-4 py-4 text-right">
                          <button
                            type="button"
                            onClick={() =>
                              handleSortChange(key as SalesByItemSortKey)
                            }
                          ...
                          >
                            ...
                            <span className="text-xs text-slate-500">
                              {getSortIndicator(key as SalesByItemSortKey)}
                            </span>
                          </button>
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-white/[0.08]">
                    {sortedItemRows.map((row, index) => {
                      const share = getItemShare(row.grossSales)
                      const rank = rankByItemKey.get(row.itemKey) ?? index + 1
                      const badge = getItemPerformanceBadge(rank, share, row.grossSales)
                        ...
                        <td className="px-4 py-4">
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500">
                              <span>{row.itemCategory || 'بدون تصنيف'}</span>
