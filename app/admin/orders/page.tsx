...
type OrdersFilterKey = OrderFilter | 'new' | 'cancelled'
...
export default function OrdersPage() {
  ...
  const updateStatus = async (order: OrderRecord, status: OrderStatus) => {
    ...
    if (!access.tenantId) {
      showError('ØªØ¹Ø°Ø± ØªØ­Ø¯ÙŠØ¯ Ù†Ø·Ø§Ù‚ Ø§Ù„Ù…Ù†Ø´Ø£Ø© Ù„ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø·Ù„Ø¨')
      return
    }
        ...
        <section dir="rtl" className="w-full max-w-full space-y-5 text-white">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {[
              ...
              { label: 'تم التسليم', value: stats.closedCount, filterKey: 'closed' as const, tone: 'from-emerald-500/20 to-green-500/5', icon: 'M5 12l4 4L19 6' },
              ...
              <button
                ...
                className={`cursor-pointer rounded-[24px] border bg-gradient-to-br ${card.tone} p-4 text-right shadow-[0_0_35px_rgba(0,0,0,0.18)] transition-all duration-150 hover:-translate-y-0.5 hover:border-cyan-300/40 hover:shadow-[0_0_32px_rgba(34,211,238,0.14)] active:scale-[0.98] ${
                  isActive
          ...
          <div className="rounded-[28px] border border-cyan-300/10 bg-[#07111d]/90 p-4 backdrop-blur-xl">
            <div className="grid gap-3 xl:grid-cols-[1.2fr_1fr_minmax(220px,0.7fr)_auto]">
              <div className="relative">
                ...
                <input
                  ...
                  className="h-12 w-full rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.03] px-4 pr-11 text-right text-sm font-bold text-white outline-none transition placeholder:text-slate-500 focus:border-cyan-300/55 focus:ring-2 focus:ring-cyan-300/15"
                />
              ...
              <div>
                {isSystemAdmin ? (
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex h-11 items-center rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.03] px-4 text-sm font-black text-slate-200">
          ...
          <div className="max-w-full overflow-hidden rounded-[30px] border border-cyan-300/10 bg-[#07111d]/95 shadow-[0_0_45px_rgba(34,211,238,0.06)] backdrop-blur-xl">
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1050px] table-fixed text-right" dir="rtl">
                  <colgroup>
                  ...
                  <thead className="bg-[#0b1626]/90">
                    <tr className="border-b border-cyan-300/10 text-xs font-black text-slate-300">
                      <th className="px-5 py-4">رقم الفاتورة / الطلب</th>
                        ...
                        >
                          <td className="px-5 py-5 align-middle">
                            <div className="space-y-1">
                              <p className="font-black text-white">{order.invoice_number || order.order_number}</p>
                              <p className="text-xs font-bold text-slate-500">{order.order_number}</p>
                            </div>
                          </td>
                          <td className="px-5 py-5 align-middle">
                            <p className="truncate font-bold text-slate-100">{fixArabic(order.customer_name)}</p>
                          </td>
                          <td className="px-5 py-5 align-middle">
                            <p className="font-bold text-slate-300">{order.customer_phone}</p>
                          </td>
                          <td className="px-5 py-5 align-middle">
                            <p className="truncate font-bold text-slate-300">{getOrderBranchLabel(order)}</p>
                          </td>
                          <td className="px-5 py-5 align-middle">
                            <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-black ${statusUi.badgeClassName}`}>
                              <span className={`h-2 w-2 rounded-full ${statusUi.dotClassName}`} />
                              {statusUi.label}
                            </span>
                          </td>
                          <td className="px-5 py-5 align-middle">
                            <div className="flex flex-wrap items-center gap-2">
                              <button
                                ...
                                className="h-10 rounded-full border border-sky-300/35 bg-sky-500/15 px-4 text-xs font-black text-sky-100 transition hover:bg-sky-500/25 hover:shadow-[0_0_18px_rgba(14,165,233,0.2)] disabled:cursor-not-allowed disabled:border-slate-500/25 disabled:bg-slate-500/10 disabled:text-slate-500 disabled:hover:shadow-none"
                              >
                                تم التجهيز
                              </button>
                              <button
                                ...
                                className="h-10 cursor-not-allowed rounded-full border border-rose-300/25 bg-rose-500/10 px-4 text-xs font-black text-rose-300/60 opacity-70"
                              >
                                إلغاء الطلب
                              </button>
                              <button
                                ...
                                className="h-10 rounded-full border border-emerald-300/35 bg-emerald-500/15 px-4 text-xs font-black text-emerald-100 transition hover:bg-emerald-500/25 hover:shadow-[0_0_18px_rgba(16,185,129,0.2)] disabled:cursor-not-allowed disabled:border-slate-500/25 disabled:bg-slate-500/10 disabled:text-slate-500 disabled:hover:shadow-none"
                              >
                                تم التسليم
                              </button>
                            </div>
