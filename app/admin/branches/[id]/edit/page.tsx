...
export default function EditBranchPage() {
  ...
  useEffect(() => {
    ...
    async function loadBranch() {
      try {
        ...
        if (!branch) {
          throw new Error('تعذر العثور على الفرع المطلوب')
        }
  ...
  }, [accessLoading, allowed, isSystemAdmin, branchId])
    ...
    try {
      ...
      const response = await fetch('/api/admin/branches', {
        ...
        body: JSON.stringify({
          ...
          display_branch_name: form.display_branch_name,
        }),
      })
      ...
      setErrorMessage(
        error instanceof Error ? error.message : 'تعذر حفظ الفرع'
      )
    } finally {
      setSaving(false)
    }
  ...
  if (accessLoading || loadingBranch) {
    return (
            ...
            <div className="flex items-center gap-4 text-right">
              ...
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-slate-400">
                  حدّث بيانات الفرع ورابط الموقع وأسماء العرض المستخدمة في
                  الرسائل.
                </p>
              </div>
            </div>
          ...
          >
            <div className="mb-6 text-right">
              ...
              <p className="mt-2 text-sm text-slate-400">
                يتم حفظ التعديل على هذا الفرع فقط دون تغيير حالة التفعيل أو
                الفروع الأخرى.
              </p>
            </div>
