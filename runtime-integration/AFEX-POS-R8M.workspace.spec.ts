...
const row = (entry: string[]) => `<button type="button" class="pos-invoice-ledger-row" data-kind="${entry[0]}" data-selected="${entry[0] === 'cash-change'}"><span class="is-identity"><strong>${entry[1]}</strong><small>${entry[2]}</small></span><time>03:30 م</time><span>${entry[3]}</span><b>${entry[4]}</b><i>${entry[5]}</i></button>`
...
for (const theme of ['light','dark']) test(`R8M ${theme} ledger geometry and financial states`, async ({ page, browserName }) => {
  ...
  for (const size of sizes) {
    ...
    const metrics = await page.evaluate(() => { const list=document.querySelector<HTMLElement>('.pos-invoice-ledger')!; const buttons=[...document.querySelectorAll<HTMLElement>('button')]; list.scrollTop=list.scrollHeight; const ledgerRows=[...document.querySelectorAll<HTMLElement>('.pos-invoice-ledger-row')]; const last=ledgerRows.at(-1)!; const listRect=list.getBoundingClientRect(); const lastRect=last.getBoundingClientRect(); return { overflow: document.documentElement.scrollWidth-window.innerWidth, listOwners: Number(list.scrollHeight>list.clientHeight), documentScroll: document.documentElement.scrollHeight-window.innerHeight, shortTargets: buttons.filter((button)=>{const style=getComputedStyle(button); const r=button.getBoundingClientRect(); return style.display!=='none'&&style.visibility!=='hidden'&&Number(style.opacity)>0&&(r.width<44||r.height<44)}).length, lastReachable: lastRect.bottom<=listRect.bottom+1&&lastRect.top>=listRect.top-1 } })
    expect(metrics).toMatchObject({ overflow: 0, listOwners: 1, documentScroll: 0, shortTargets: 0, lastReachable: true })
  }
