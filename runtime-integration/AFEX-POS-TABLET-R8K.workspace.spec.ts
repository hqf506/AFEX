...
const evidence = 'C:/AFEX-Evidence/R8K.1-AUTOMATION-SCREENSHOTS'
...
for (const theme of ['dark', 'light']) {
  test(`R8K.1 ${theme} complete master-list access and action geometry`, async ({ page, browserName }) => {
    ...
    for (const size of sizes) {
      ...
      expect(metrics).toMatchObject({ overflow: 0, rootBottom: metrics.viewportBottom, listOwners: 1, finalFullyVisible: true, finalSelected: true, selectedDetails: '02-0040', actionVisible: true, shortTargets: 0, centerClickable: true, shellScroll: 0 })
