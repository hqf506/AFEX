...
export function ProviderMobileBottomNavigation() {
  ...
  return (
    <MobileBottomNav
      ...
      items={[
        { key: 'tickets', label: 'التذاكر', active: shortcut === 'all', disabled: pending, onSelect: () => selectShortcut('all'), icon: <span className="text-base">▤</span> },
        { key: 'assigned', label: 'مسندة إليّ', active: shortcut === 'assigned', disabled: pending, onSelect: () => selectShortcut('assigned'), icon: <span className="text-base">◎</span> },
        { key: 'critical', label: 'حرجة', active: shortcut === 'critical', disabled: pending, onSelect: () => selectShortcut('critical'), icon: <span className="text-base">!</span> },
      ]}
    />
  )
}
