...
export function ProviderSupportConsole({ variant = 'provider' }: { variant?: 'provider' | 'developer' }) {
  ...
  return (
    <main dir="rtl" className={`mx-auto w-full min-w-0 space-y-5 ${variant === 'developer' ? 'max-w-none' : 'max-w-[1500px]'}`}>
      ...
      <AdminGlassSection>
        <div className={`grid gap-3 md:grid-cols-2 ${variant === 'developer' ? 'xl:grid-cols-4 2xl:grid-cols-[minmax(260px,1.8fr)_repeat(6,minmax(130px,1fr))_auto]' : 'xl:grid-cols-[minmax(210px,1.5fr)_repeat(6,minmax(135px,1fr))_auto]'}`}>
