...
export function PosTabletFrame({
  ...
  return (
    <PosTabletFrameContext.Provider value={contextValue}>
      ...
      <div className="hidden min-h-screen w-screen overflow-auto bg-black xl:flex xl:items-center xl:justify-center">
        <div
          className={`relative shrink-0 transition-all duration-200 ease-out ${frameWidthClass} ${desktopScreenHeightClass} ${
            hydrated ? 'opacity-100 transition-opacity duration-150' : 'opacity-0'
          }`}
        >
          <div
            className="relative h-full overflow-hidden rounded-[24px] border border-black/30 bg-black p-[2px] shadow-[0_30px_100px_rgba(0,0,0,0.65)] ring-1 ring-white/10 transition-all duration-200 ease-out"
          >
            <span
              aria-hidden="true"
            ...
            />
            <div
              className="h-full w-full overflow-hidden rounded-[20px] bg-white transition-all duration-200 ease-out"
            >
              <div
