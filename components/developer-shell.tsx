  ...
  return (
    <DeveloperSupportNotificationsProvider>
      <div dir="rtl" className="min-h-screen bg-[#030714] text-white">
        <div className={`mx-auto grid w-full min-w-0 gap-4 p-3 sm:p-4 xl:grid-cols-[230px_minmax(0,1fr)] xl:px-5 xl:py-5 ${wide ? 'max-w-[1920px] 2xl:px-6' : 'max-w-[1700px]'}`}>
          ...
          <main className="min-w-0 w-full">
            <header className="mb-4 flex min-h-14 items-center justify-between gap-2 rounded-[20px] border border-white/10 bg-[#07111f]/80 px-3 py-2 backdrop-blur-xl">
              <button ref={menuTriggerRef} type="button" aria-label="فتح قائمة مركز المطور" aria-expanded={navigationOpen} aria-controls="developer-mobile-navigation" onClick={() => setNavigationOpen(true)} className="grid size-11 shrink-0 place-items-center rounded-xl border border-cyan-300/25 bg-cyan-300/10 text-cyan-100 xl:hidden"><svg viewBox="0 0 24 24" className="size-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16" /></svg></button>
